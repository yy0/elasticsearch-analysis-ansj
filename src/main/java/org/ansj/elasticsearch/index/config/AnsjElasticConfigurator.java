...
public class AnsjElasticConfigurator {
    ...
    public static void init(Settings indexSettings, Settings settings) {
        Environment environment = new Environment(indexSettings);
        init(environment, settings);
    }

    public static void init(Environment env, Settings settings) {
        if (isLoaded()) {
            return;
        }
        environment = env;
    ...
    private static void initConfigPath(Settings settings) {
        ...
        //用户自定义辞典
        File path = new File(environment.pluginsFile().toString(), settings.get("user_path", DEFAULT_USER_LIB_PATH));
        MyStaticValue.userLibrary = path.getAbsolutePath();
        logger.debug("用户词典路径:{}", MyStaticValue.userLibrary);
        //用户自定义辞典
        path = new File(environment.pluginsFile().toString(), settings.get("ambiguity", DEFAULT_AMB_FILE_LIB_PATH));
    ...
    private static void loadFilter(Settings settings) {
        ...
        File stopLibrary = new File(environment.pluginsFile().toString(), stopLibraryPath);
